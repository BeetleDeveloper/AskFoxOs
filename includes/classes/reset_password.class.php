...
class Reset_Password {
	...
	public static function set_confirm_email_link($email) {
		...
		if (!$email) {
			...
			$session = new Session();
			$session->message("<div class='alert alert-success'><button type='button' class='close' data-dismiss='alert'>×</button>Confirmar código enviado a{$email}.</div>");
			redirect_to('login');
		} else {
	...
	public static function check_confirm_link($email, $hash) {
		...
		if ($rows) {
			...
			if ($rows) {
				...
				$session = new Session();
				$session->message("<div class='notification-box error-notification-box'><p>Lo sentimos, pero no se encontró ningún vínculo contraseña.</p><a href='#' class='notification-close error-notification-close'>x</a></div><!--.notification-box .notification-box-error end-->");
				redirect_to('login');
			}
		} else {
			// Throw error back to the user.
			$session = new Session();
			$session->message("<div class='notification-box error-notification-box'><p>Esa cuenta no existe.</p><a href='#' class='notification-close error-notification-close'>x</a></div><!--.notification-box .notification-box-error end-->");
			redirect_to('login');
		}
	}
	...
	public static function check_resend_code($email) {
		...
		if ($rows) {
			...
			if ($rows) {
				...
				$session = new Session();
				$session->message("<div class='notification-box error-notification-box'><p>No hay enlace de contraseña encontrado</p><a href='#' class='notification-close error-notification-close'>x</a></div><!--.notification-box .notification-box-error end-->
				");
				redirect_to('login');
			}
		} else {
			// Throw error back to the user.
			$session = new Session();
			$session->message("<div class='notification-box error-notification-box'><p>Ese código no existe en nuestra base de datos.</p><a href='#' class='notification-close error-notification-close'>x</a></div><!--.notification-box .notification-box-error end-->
			");
			redirect_to('login');
		}
	}
	...
	public static function set_password($email, $hash) {
		...
		$session = new Session();
		$session->message("<div class='alert alert-success'><button type='button' class='close' data-dismiss='alert'>×</button>Su nueva contraseña ha sido enviada por correo electrónico a {$email}.</div>");
